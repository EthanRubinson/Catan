...
module Bot = functor (S : Soul) -> struct
  ...
  let weighted_valid_next_town_spot_list_for_point_with_weights (b:board) (p:point) : (float * (point * point)) list =
    let (m,s,_,_,_) = b in
    let (hl,_) = m in
    ...
    let needed_roads = List.map (fun elem -> let (we,o) = elem in (we,(p,o))) weighted_next_town_spot_list in
    (List.filter (fun elem -> valid_road_position rlist (fst (snd elem)) (snd (snd elem))) needed_roads)
  ...
  let handle_request ((b,p,t,n) : state) : move =
    ...
    | InitialRequest -> begin 
        ...
        let rec do_move = function
          | [] -> -1
          | (w,p)::t ->  let (_,s,_,_,_) = b in let (ilist,_) = s in if (valid_town_spot ilist p) && (List.length (weighted_valid_next_town_spot_list_for_point b p) > 0) then p else do_move t
        in
    ...
    | ActionRequest -> begin
        ...
        let has_knights = (List.length (List.filter (fun elem -> elem == Knight) hand)) > 1 in
        let has_yop = (List.length (List.filter (fun elem -> elem == YearOfPlenty) hand)) > 0 in
        let has_monopoly = (List.length (List.filter (fun elem -> elem == Monopoly) hand)) > 0 in
        let index = ref (-1) in
              ...
              if can_afford_cost pinv cCOST_TOWN then begin
                ...
                | None -> (* We need roads *) begin
                    ...
                    if can_afford_cost pinv cCOST_ROAD then begin
                      ...
                      let where = List.sort (fun e f -> (compare e f) * -1) (List.flatten (List.map (weighted_valid_next_town_spot_list_for_point_with_weights b) road_endpoints)) in
                      match where with
                      | [] -> begin
                          let yol = (List.flatten (List.map (fun elem -> (List.map (fun n -> (elem,n)) (valid_roads_for_point_and_color elem rlist self))) road_endpoints)) in
                          if List.length yol > 0 then 
                            Action(BuyBuild(BuildRoad(self,(List.hd yol))))
                          else begin 
                            if t.cardplayed == false && (has_yop || has_monopoly) then begin
                              let least_res = let (b,w,o,g,l) = pinv in let mmm =  (min(min(min(min b w) o) g) l) in (if mmm = b then Brick else if mmm = w then Wool else if mmm = o then Ore else if mmm = g then Grain else Lumber) in
                              if has_yop then begin
                                Action(PlayCard(PlayYearOfPlenty(least_res,Some(least_res))))
                              end
                              else begin
                                Action(PlayCard(PlayMonopoly(least_res)))
                              end


                            end
                            else begin
                              if can_afford_cost pinv cCOST_CARD then begin
                                Action(BuyBuild(BuildCard))
                              end
                              else begin

                                if has_knights && t.cardplayed == false then begin
                                  let robMove = (match (calc_robber_placement_ranked_options b self p) with
                                      | [] -> begin (debug "[INFO] Want to move robber, but no ideal placement was found. Defaulting to (0,None)");
                                          (0, None) 
                                        end
                      ...
                      | h::t -> Action(BuyBuild(BuildRoad(self,(snd h))))
                ...
                let ratio  = ((float_of_int (List.length (List.filter (fun elem -> let (c,(p1,p2)) = elem in c == self) rlist)) ) /. (float_of_int (total_towns_for_player ilist self + total_cities_for_player ilist self))) <= 3.0 in
